...
BYTETracker::BYTETracker(const int &frame_rate, const int &track_buffer, const float &track_thresh, const float &high_thresh, const float &match_thresh) {
...
vector<STrack> BYTETracker::update(const vector<DetectRes> &objects) {
    ...
    if (objects.size() > 0) {
        for (int i = 0; i < objects.size(); i++) {
            vector<float> tlbr_;
            tlbr_.resize(4);
            tlbr_[0] = objects[i].tx;
            tlbr_[1] = objects[i].ty;
            tlbr_[2] = objects[i].tx + objects[i].w;
            tlbr_[3] = objects[i].ty + objects[i].h;
