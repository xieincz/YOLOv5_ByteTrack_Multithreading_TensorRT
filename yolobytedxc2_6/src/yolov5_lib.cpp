...
//#define NMS_THRESH 0.5 //offical is 0.4
...
int yolov5_trt_detect(void *h, cv::Mat &img, const float& threshold,std::vector<DetectRes>& det,const float& nms_thresh)
{
    ...
    for(i = 0 ; i < res.size(); i++){
        ...
        //int class_id;
        //float conf;
        //if((int)res[i].class_id!=0) continue;//only detect person//�Ѿ���nms�й�����
        cv::Rect r = get_rect(img, res[i].bbox);
 
        //DetectBox dd(r.x,r.y,r.x + r.width,r.y + r.height,(float)res[i].conf,(int)res[i].class_id);
        //det.push_back(dd);
        //det.emplace_back(r.x,r.y,r.x + r.width,r.y + r.height,(float)res[i].conf,(int)res[i].class_id);
        det.emplace_back(r.x,r.y,r.width,r.height,(int)res[i].class_id,(float)res[i].conf);
    }
    return 1;
}
