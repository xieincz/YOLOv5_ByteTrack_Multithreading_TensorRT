...
/*
vector<vector<float>> YoloByteAPI::detectOneImage(char *image_path) {
    cv::Mat src_img = cv::imread(image_path);
    yolov5_trt_detect(trt_engine, src_img, conf_thresh, dets,nms_thresh);
    vector<STrack> output_stracks = tracker->update(dets);
    vector<vector<float>> result;
    for (const auto &box : output_stracks) {
        const vector<float> &tlwh = box.tlwh;
        result.emplace_back(vector<float>{box.track_id, tlwh[0], tlwh[1], tlwh[2], tlwh[3], box.score});
    }
    return result;
}
*/
void YoloByteAPI::writeResultForModel(FILE *fp, const vector<STrack> &output_stracks, const int &frame_id) const
{
    for (const STrack &d : output_stracks)
    {
        const vector<float> &tlwh = d.tlwh;
        fprintf(fp, "%d,%d,%d,%d,%d,%d,%f\n", frame_id, int(d.track_id), max(int(tlwh[0]), 0), max(int(tlwh[1]), 0), int(tlwh[2]), int(tlwh[3]), d.score);
    }
}
...
int YoloByteAPI::detectImageDirForModel(char *images_dir, char *output_txt_path)
{
    vector<string> file_names;
    read_files_in_dir(images_dir, file_names);
    FILE *fp = fopen(output_txt_path, "w");
    for (int i = 0, size = file_names.size(); i < size; i++)
    {
        // cout<<string(images_dir) + "/" + file_names[i]<<endl;
        cv::Mat src_img = cv::imread(string(images_dir) + "/" + file_names[i]);
        yolov5_trt_detect(trt_engine, src_img, conf_thresh, dets, nms_thresh);
        vector<STrack> output_stracks = tracker->update(dets);
        writeResultForModel(fp, output_stracks, i + 1);
        /*for (const STrack &d : output_stracks) {
            const vector<float> &tlwh = box.tlwh;
            fprintf(fp, "%d,%d,%d,%d,%d,%d,%f\n", i + 1, int(d.track_id), max(int(tlwh[0]), 0), max(int(tlwh[1]), 0), int(tlwh[2]), int(tlwh[3]), d.score);
        }*/
    }
    fclose(fp);
    return 0;
...
int YoloByteAPI::detectImageDirForModelRenew(char *images_dir, char *output_txt_path)
{
    vector<string> file_names;
    BYTETracker newtracker(frame_rate, track_buffer, track_thresh, high_thresh, match_thresh);
    read_files_in_dir(images_dir, file_names);
    FILE *fp = fopen(output_txt_path, "w");
    for (int i = 0, size = file_names.size(); i < size; i++)
    {
        cv::Mat src_img = cv::imread(string(images_dir) + "/" + file_names[i]);
        yolov5_trt_detect(trt_engine, src_img, conf_thresh, dets, nms_thresh);
        vector<STrack> output_stracks = newtracker.update(dets);
        writeResultForModel(fp, output_stracks, i + 1);
    }
    fclose(fp);
    return 0;
...
int YoloByteAPI::detectImageDirForModelRenew(char *images_dir, char *output_txt_path, char *output_image_dir)
{
    vector<string> file_names;
    BYTETracker newtracker(frame_rate, track_buffer, track_thresh, high_thresh, match_thresh);
    read_files_in_dir(images_dir, file_names);
    ...
    for (int i = 0, size = file_names.size(); i < size; i++)
    {
        ...
        vector<STrack> output_stracks = newtracker.update(dets);
        writeResultForModel(fp, output_stracks, i + 1);
        drawResultForModel(src_img, output_stracks, string(output_image_dir) + "/out_" + file_names[i]);
    }
    fclose(fp);
    return 0;
}

int YoloByteAPI::detectImageDirForModel(char *images_dir, char *output_txt_path, char *output_image_dir)
{
    vector<string> file_names;
    read_files_in_dir(images_dir, file_names);
    FILE *fp = fopen(output_txt_path, "w");
...
bool YoloByteAPI::check_crash(const int &px, const int &py, const vector<pair<int, int>> &polygon)
{
    ...
    for (int i = 0; i < l; i++)
    {
        ...
        { // find horizontal edges of polygon
            ...
            { // if point is on right side of edge
                is_in = !is_in;
            }
        }
    }
...
int YoloByteAPI::detectImageDirForAlgorithm(const char *video_path, const char *polygon_str, const int &frame_count_thresh, int skip_num)
{
    ...
    while (video_cap.read(src_img))
    {
        ...
        for (const STrack &d : output_stracks)
        {
            ...
            {
                if (record_id.find(track_id) == record_id.end())
                {   // not found
                    //if (record_id.count(track_id) == 0) {// not found
                ...
                need_remove_ids.emplace_back(track_id);
            }*/
        }
        ...
        for (const auto &id : need_remove_ids) {
            record_id.erase(id);
        }*/
        //bool leave=false;
        for (const auto &it : record_id)
...
void YoloByteAPI::res()
{
    ...
    while (!end)
    {
        ...
        if (track_end && stracks_queue.empty())
        {
            ...
            res_end = true;
            record_id.clear();
            //cout<<"cv_res.notify_all();"<<endl;
            cv_res.notify_all();
            //cout<<"try to wake up out"<<endl;
            delete tracker;
            tracker = new BYTETracker(frame_rate, track_buffer, track_thresh, high_thresh, match_thresh);
            continue;
        }
